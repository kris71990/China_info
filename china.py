	...
	while True:
		...
		if x == 'q':
			...
			print_more_info_autreg(autregions[0], aut_abbrev[0], autregion_pop[0], autregion_pop_rank[0],
									...
									autregion_cap[0], autregion_cap_pop[0])
			continue
		elif x == "GZ" or x == "Gz" or x == "gz":
			print_more_info_prov(provinces[4], province_abbrev[4], province_pop[4],
							prov_pop_rank[4], prov_area[4], prov_area_rank[4], 
							prov_capital[4], prov_capital[4], prov_cap_pop[4])
			continue
		elif x == "HI" or x == "Hi" or x == "hi":
			print_more_info_prov(provinces[5], province_abbrev[5], province_pop[5],
							prov_pop_rank[5], prov_area[5], prov_area_rank[5], 
							prov_capital[5], prov_capital[5], prov_cap_pop[5])
			continue
		elif x == "HE" or x == "He" or x == "he":
			print_more_info_prov(provinces[6], province_abbrev[6], province_pop[6],
							prov_pop_rank[6], prov_area[6], prov_area_rank[6], 
							prov_capital[6], prov_capital[6], prov_cap_pop[6])
			continue
		elif x == "HL" or x == "Hl" or x == "hl":
			print_more_info_prov(provinces[7], province_abbrev[7], province_pop[7],
							prov_pop_rank[7], prov_area[7], prov_area_rank[7], 
							prov_capital[7], prov_capital[7], prov_cap_pop[7])
			continue
		elif x == "HA" or x == "Ha" or x == "ha":
			print_more_info_prov(provinces[8], province_abbrev[8], province_pop[8],
							prov_pop_rank[8], prov_area[8], prov_area_rank[8], 
							prov_capital[8], prov_capital[8], prov_cap_pop[8])
			continue
		elif x == "HK" or x == "Hk" or x == "hk":
			print_more_info_admreg(admregions[0], adm_abbrev[0], admregion_pop[0],
							admregion_pop_rank[0], admregion_area[0], admregion_area_rank[0], 
							admregion_capital[0], admregion_capital[0], admregion_pop[0])
			continue
		elif x == "HB" or x == "Hb" or x == "hb":
			print_more_info_prov(provinces[9], province_abbrev[9], province_pop[9],
							prov_pop_rank[9], prov_area[9], prov_area_rank[9], 
							prov_capital[9], prov_capital[9], prov_cap_pop[9])
			continue
		elif x == "HN" or x == "Hn" or x == "hn":
			print_more_info_prov(provinces[10], province_abbrev[10], province_pop[10],
							prov_pop_rank[10], prov_area[10], prov_area_rank[10], 
							prov_capital[10], prov_capital[10], prov_cap_pop[10])
			continue
		elif x == "NM" or x == "Nm" or x == "nm":
			print_more_info_autreg(autregions[1], aut_abbrev[1], autregion_pop[1], autregion_pop_rank[1],
									autregion_area[1], autregion_area_rank[1], autregion_cap[1],
									autregion_cap[1], autregion_cap_pop[1])
			continue	
		elif x == "JS" or x == "Js" or x == "js":
			print_more_info_prov(provinces[11], province_abbrev[11], province_pop[11],
							prov_pop_rank[11], prov_area[11], prov_area_rank[11], 
							prov_capital[11], prov_capital[11], prov_cap_pop[11])
			continue
		elif x == "JX" or x == "Jx" or x == "jx":
			print_more_info_prov(provinces[12], province_abbrev[12], province_pop[12],
							prov_pop_rank[12], prov_area[12], prov_area_rank[12], 
							prov_capital[12], prov_capital[12], prov_cap_pop[12])
			continue
		elif x == "JL" or x == "Jl" or x == "jl":
			print_more_info_prov(provinces[13], province_abbrev[13], province_pop[13],
							prov_pop_rank[13], prov_area[13], prov_area_rank[13], 
							prov_capital[13], prov_capital[13], prov_cap_pop[13])
			continue
		elif x == "LN" or x == "Ln" or x == "ln":
			print_more_info_prov(provinces[14], province_abbrev[14], province_pop[14],
							prov_pop_rank[14], prov_area[14], prov_area_rank[14], 
							prov_capital[14], prov_capital[14], prov_cap_pop[14])
			continue
		elif x == "MC" or x == "Mc" or x == "mc":
			print_more_info_admreg(admregions[1], adm_abbrev[1], admregion_pop[1],
							admregion_pop_rank[1], admregion_area[1], admregion_area_rank[1], 
							admregion_capital[1], admregion_capital[1], admregion_pop[1])
			continue
		elif x == "NX" or x == "Nx" or x == "nx":
			print_more_info_autreg(autregions[2], aut_abbrev[2], autregion_pop[2], autregion_pop_rank[2],
									autregion_area[2], autregion_area_rank[2], autregion_cap[2],
									autregion_cap[2], autregion_cap_pop[2])
			continue
		elif x == "QH" or x == "Qh" or x == "qh":
			print_more_info_prov(provinces[15], province_abbrev[15], province_pop[15],
							prov_pop_rank[15], prov_area[15], prov_area_rank[15], 
							prov_capital[15], prov_capital[15], prov_cap_pop[15])
			continue
		elif x == "SN" or x == "Sn" or x == "sn":
			print_more_info_prov(provinces[16], province_abbrev[16], province_pop[16],
							prov_pop_rank[16], prov_area[16], prov_area_rank[16], 
							prov_capital[16], prov_capital[16], prov_cap_pop[16])
			continue
		elif x == "SD" or x == "Sd" or x == "sd":
			print_more_info_prov(provinces[17], province_abbrev[17], province_pop[17],
							prov_pop_rank[17], prov_area[17], prov_area_rank[17], 
							prov_capital[17], prov_capital[17], prov_cap_pop[17])
			continue
		elif x == "SH" or x == "Sh" or x == "sh":
			print_more_info_muni(municipalities[2], mun_abbrev[2], muni_pop[2], muni_pop_rank[2],
									muni_area[2], muni_area_rank[2], mun_capital[2], mun_capital[2],
									muni_cap_pop[2])
			continue
		elif x == "SX" or x == "Sx" or x == "sx":
			print_more_info_prov(provinces[18], province_abbrev[18], province_pop[18],
							prov_pop_rank[18], prov_area[18], prov_area_rank[18], 
							prov_capital[18], prov_capital[18], prov_cap_pop[18])
			continue
		elif x == "SC" or x == "Sc" or x == "sc":
			print_more_info_prov(provinces[19], province_abbrev[19], province_pop[19],
							prov_pop_rank[19], prov_area[19], prov_area_rank[19], 
							prov_capital[19], prov_capital[19], prov_cap_pop[19])
			continue
		elif x == "TW" or x == "Tw" or x == "tw":
			print_more_info_prov(provinces[20], province_abbrev[20], province_pop[20],
							prov_pop_rank[20], prov_area[20], prov_area_rank[20], 
							prov_capital[20], prov_capital[20], prov_cap_pop[20])
			continue
		elif x == "TJ" or x == "Tj" or x == "tj":
			print_more_info_muni(municipalities[3], mun_abbrev[3], muni_pop[3], muni_pop_rank[3],
									muni_area[3], muni_area_rank[3], mun_capital[3], mun_capital[3],
									muni_cap_pop[3])
			continue
		elif x == "XZ" or x == "Xz" or x == "xz":
			print_more_info_autreg(autregions[3], aut_abbrev[3], autregion_pop[3], autregion_pop_rank[3],
									autregion_area[3], autregion_area_rank[3], autregion_cap[3],
									autregion_cap[3], autregion_cap_pop[3])
			continue
		elif x == "XJ" or x == "Xj" or x == "xj":
			print_more_info_autreg(autregions[4], aut_abbrev[4], autregion_pop[4], autregion_pop_rank[4],
									autregion_area[4], autregion_area_rank[4], autregion_cap[4],
									autregion_cap[4], autregion_cap_pop[4])
			continue
		elif x == "YN" or x == "Yn" or x == "yn":
			print_more_info_prov(provinces[21], province_abbrev[21], province_pop[21],
							prov_pop_rank[21], prov_area[21], prov_area_rank[21], 
							prov_capital[21], prov_capital[21], prov_cap_pop[21])
			continue
		elif x == "ZJ" or x == "Zj" or x == "zj":
			print_more_info_prov(provinces[22], province_abbrev[22], province_pop[22],
							prov_pop_rank[22], prov_area[22], prov_area_rank[22], 
							prov_capital[22], prov_capital[22], prov_cap_pop[22])
			continue
		else:
...
def print_more_info_prov(p, pab, pp, ppr, pa, par, pc, pc2, pcp):
	print "\nHere is some information about %s:\n" % p
	print "Abbreviation: %s" % pab
	print "Population: %s (%s)" % (pp, ppr)
	print "Area: %s square km (%s)" % (pa, par)
	print "Capital: %s" % pc
	print "Population of %s: %s" % (pc2, pcp)
	
def print_more_info_muni(m, mab, mp, mpr, ma, mar, mc, mc2, mcp):
	print "\nHere is some information about %s:\n" % m
	print "Abbreviation: %s" % mab
	print "Population: %s (%s)" % (mp, mpr)
	print "Area: %s square km (%s)" % (ma, mar)
	print "Capital: %s" % mc
	print "Population of %s: %s" % (mc2, mcp)
	
def print_more_info_autreg(aur, aurab, aurp, aurpr, aura, aurar, aurc, aurc2, aurcp):
	...
	print "Population of %s: %s" % (aurc2, aurcp)

def print_more_info_admreg(admr, admrab, admrp, admrpr, admra, admrar, admrc, admrc2, admrcpr):
	print "\nHere is some information about %s:\n" % admr
	print "Abbreviation: %s" % admrab
	print "Population: %s (%s)" % (admrp, admrpr)
	print "Area: %s square km (%s)" % (admra, admrar)
	print "Capital: %s" % admrc
	print "Population of %s: %s" % (admrc2, admrcpr)
